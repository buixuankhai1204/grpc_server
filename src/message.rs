...
pub mod message_client {
    ...
    impl MessageClient<tonic::transport::Channel> {
        ...
        <T::ResponseBody as Body>::Error: Into<StdError> + std::marker::Send,
    {
        ...
        pub fn with_interceptor<F>(
            inner: T,
            interceptor: F,
        ) -> MessageClient<InterceptedService<T, F>>
        where
        ...
        {
            MessageClient::new(InterceptedService::new(inner, interceptor))
        }
        ...
        pub async fn send_message(
            &mut self,
            request: impl tonic::IntoStreamingRequest<Message = super::MessageRequest>,
        ) -> std::result::Result<
...
pub mod message_server {
    #![allow(unused_variables, dead_code, missing_docs, clippy::let_unit_value)]
    use tonic::codegen::*;
    /// Generated trait containing gRPC methods that should be implemented for use with MessageServer.
    #[async_trait]
    pub trait Message: std::marker::Send + std::marker::Sync + 'static {
    ...
    impl<T, B> tonic::codegen::Service<http::Request<B>> for MessageServer<T>
    where
        T: Message,
        ...
        fn call(&mut self, req: http::Request<B>) -> Self::Future {
            match req.uri().path() {
                "/message.Message/SendMessage" => {
                    ...
                    struct SendMessageSvc<T: Message>(pub Arc<T>);
                    impl<
                        T: Message,
                    > tonic::server::StreamingService<super::MessageRequest>
                    for SendMessageSvc<T> {
                        ...
                        fn call(
                            ...
                            request: tonic::Request<
                                tonic::Streaming<super::MessageRequest>,
                            >,
                        ) -> Self::Future {
                            let inner = Arc::clone(&self.0);
                            let fut = async move {
                                <T as Message>::send_message(&inner, request).await
                            };
                            Box::pin(fut)
                        }
                    }
    ...
    impl<T> Clone for MessageServer<T> {
        fn clone(&self) -> Self {
            let inner = self.inner.clone();
            Self {
                inner,
    ...
    pub const SERVICE_NAME: &str = "message.Message";
    impl<T> tonic::server::NamedService for MessageServer<T> {
